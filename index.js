...
case 'animehug':
				...
				case 'tulis':
					...
					teks = body.slice(7)
					reply(mess.wait)
					anu = await fetchJson(`https://api.zeks.xyz/api/magernulis?nama=${nama}&kelas=${kelas}&text=${isi}&tinta=4`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buff = await getBuffer(anu.result)
...
case 'textodeneve':
				...
				case 'wait':
					...
					break
                                					case 'dono':
if (args.length < 1) return reply('.+.. Luci +..*+� Número do meu dono😎🤙:WA.me/5522988714803\n😎🤙Youtube do meu \ndono:https://youtube.com/chael/UCOqlFzRrB8IZUWUx1cwCeUw\n ')
                                        break
case 'porno':
